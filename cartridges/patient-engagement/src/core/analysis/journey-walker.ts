...
// ---------------------------------------------------------------------------
// Walks the patient journey schema, compares two MetricsSnapshots, and
...
export function analyzeJourney(options: JourneyWalkerOptions): JourneyDiagnosticResult {
  ...
  const primaryKPI = {
    ...
    severity: classifyKPISeverity(kpiDelta, current.totalPatients),
  };
  ...
  return {
    ...
    periods,
    totalPatients: { current: current.totalPatients, previous: previous.totalPatients },
    primaryKPI,
    stageAnalysis,
    dropoffs,
    bottleneck,
...
function classifyKPISeverity(deltaPercent: number, totalPatients: number): Severity {
