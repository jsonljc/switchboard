...
import { DEFAULT_PATIENT_ENGAGEMENT_POLICIES } from "../policies.js";

describe("DEFAULT_PATIENT_ENGAGEMENT_POLICIES", () => {
  it("defines 4 policies", () => {
    expect(DEFAULT_PATIENT_ENGAGEMENT_POLICIES).toHaveLength(4);
  });

  it("all policies have required fields", () => {
    for (const policy of DEFAULT_PATIENT_ENGAGEMENT_POLICIES) {
      expect(policy.id).toBeTruthy();
      expect(policy.name).toBeTruthy();
      expect(policy.description).toBeTruthy();
      expect(policy.cartridgeId).toBe("patient-engagement");
  ...
  it("policies are ordered by priority", () => {
    const priorities = DEFAULT_PATIENT_ENGAGEMENT_POLICIES.map((p) => p.priority);
  ...
  it("consent policy (priority 1) denies outbound communication without consent", () => {
    const consent = DEFAULT_PATIENT_ENGAGEMENT_POLICIES.find(
      (p) => p.id === "patient-engagement-consent-required",
    );
    ...
    expect(actionCond.value).toContain("patient-engagement.reminder.send");
  });

  it("review response policy (priority 5) requires elevated approval", () => {
    const review = DEFAULT_PATIENT_ENGAGEMENT_POLICIES.find(
      (p) => p.id === "patient-engagement-review-elevated-approval",
    );
  ...
  it("booking policy (priority 10) requires standard approval for booking actions", () => {
    const booking = DEFAULT_PATIENT_ENGAGEMENT_POLICIES.find(
      (p) => p.id === "patient-engagement-booking-approval",
    );
    ...
    expect(actionCond.value).toContain("patient-engagement.appointment.book");
    expect(actionCond.value).toContain("patient-engagement.appointment.cancel");
    expect(actionCond.value).toContain("patient-engagement.appointment.reschedule");
  });

  it("treatment policy (priority 20) requires standard approval for treatment logging", () => {
    const treatment = DEFAULT_PATIENT_ENGAGEMENT_POLICIES.find(
      (p) => p.id === "patient-engagement-treatment-approval",
    );
