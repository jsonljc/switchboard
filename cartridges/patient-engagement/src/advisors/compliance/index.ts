...
export const consentStatusAdvisor: JourneyFindingAdvisor = (
  _stageAnalysis, _dropoffs, _current, _previous, context,
) => {
  ...
  if (commData.consentCoverage < 0.8) {
    return [{
      ...
      recommendation: "Implement consent collection at intake. Send consent renewal reminders before expiration.",
    }];
  }
...
export const communicationFrequencyAdvisor: JourneyFindingAdvisor = (
  _stageAnalysis, _dropoffs, current, _previous, context,
) => {
  const commData = context?.communicationData;
  if (!commData) return [];
  ...
  const avgMessages = commData.totalMessagesSent / totalPatients;
...
export const escalationRateAdvisor: JourneyFindingAdvisor = (
  ...
  if (commData.escalationRate > 0.3) {
    return [{
      ...
      recommendation: "Expand objection handling trees. Review common escalation reasons and add automated responses.",
    }];
  }
...
export const dataRetentionAdvisor: JourneyFindingAdvisor = (
  ...
  const dormant = current.stages["dormant_patients"]?.count ?? 0;
  const lost = current.stages["lost_patients"]?.count ?? 0;
  const total = current.totalPatients;
  ...
  if (total > 0 && (dormant + lost) / total > 0.5) {
    return [{
      ...
      recommendation: "Review data retention policy. Consider archiving records older than retention period.",
    }];
  }
...
export const medicalClaimAdvisor: JourneyFindingAdvisor = (
  ...
  if (completed > 10 && reviews > 5) {
    return [{
      ...
      message: "Periodic reminder: audit outbound communications for medical claims and guarantees.",
