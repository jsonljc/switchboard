...
export const bookingRateAdvisor: JourneyFindingAdvisor = (
  ...
  if (bookingDropoff.currentRate < 0.3) {
    return [{
      ...
      recommendation: "Simplify booking process. Offer online self-scheduling. Reduce time from qualification to booking.",
    }];
  }
...
export const noShowAdvisor: JourneyFindingAdvisor = (
  ...
  if (currentRate > 0.2) {
    return [{
      ...
      recommendation: "Implement multi-touch reminders (24hr + 2hr before). Require confirmation. Consider deposit policy.",
    }];
  }
...
export const cancellationPatternAdvisor: JourneyFindingAdvisor = (
  ...
  if (currentRate > 0.15 && delta > 20) {
    return [{
      ...
      recommendation: "Survey cancelling patients. Review scheduling flexibility and cancellation policy.",
    }];
  }
...
export const slotUtilizationAdvisor: JourneyFindingAdvisor = (
  ...
  if (booked > 0 && completed / booked < 0.7) {
    return [{
      ...
      recommendation: "Review cancellation and no-show rates. Implement waitlist for cancelled slots.",
    }];
  }
