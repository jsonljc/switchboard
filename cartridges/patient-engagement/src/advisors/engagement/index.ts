...
export const responseTimeAdvisor: JourneyFindingAdvisor = (
  ...
  if (currentResponseMs > CRITICAL_THRESHOLD_MS) {
    return [{
      ...
      recommendation: "Implement auto-response for new inquiries. Review staffing during peak hours.",
    }];
  }
  ...
  if (currentResponseMs > SLA_THRESHOLD_MS) {
    return [{
      ...
      recommendation: "Set up notification alerts for unanswered messages. Consider automated initial responses.",
    }];
  }
...
export const followupComplianceAdvisor: JourneyFindingAdvisor = (
  ...
  if (completed > 5 && repeat / completed < 0.2) {
    return [{
      ...
      recommendation: "Strengthen post-treatment follow-up cadence. Send satisfaction surveys within 24 hours.",
    }];
  }
