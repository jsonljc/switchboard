...
describe("conversation engine", () => {
  it("should create initial state", () => {
    const state = createConversationState(simpleFlow, { contactName: "Alice" });
  ...
  it("should complete after all steps", () => {
    let state = createConversationState(simpleFlow, { contactName: "Alice" });
  ...
  it("should handle action steps with actionRequired", () => {
    ...
    const state = createConversationState(flowWithAction, { contactId: "p-123" });
    const { actionRequired } = executeNextStep(flowWithAction, state);

    expect(actionRequired).toBeDefined();
    expect(actionRequired!.actionType).toBe("customer-engagement.appointment.book");
    expect(actionRequired!.parameters.contactId).toBe("p-123");
  });
});
