...
describe("resolveAgent", () => {
  it("should route lead actions to intake agent", () => {
    ...
    const agent = resolveAgent("customer-engagement.lead.qualify", agents);
  ...
  it("should route objection handling to intake agent", () => {
    const agents = makeAgentMap();
    const agent = resolveAgent("customer-engagement.conversation.handle_objection", agents);
    expect(agent).not.toBeNull();
    expect(agent!.type).toBe("intake");
  });
  ...
  it("should route cadence actions to retention agent", () => {
    const agents = makeAgentMap();
    const agent = resolveAgent("customer-engagement.cadence.start", agents);
    expect(agent).not.toBeNull();
  ...
  it("should return null for pipeline diagnostic (direct action)", () => {
    const agents = makeAgentMap();
    const agent = resolveAgent("customer-engagement.pipeline.diagnose", agents);
    expect(agent).toBeNull();
  });

  it("should return null for LTV scoring (direct action)", () => {
    const agents = makeAgentMap();
    const agent = resolveAgent("customer-engagement.contact.score_ltv", agents);
    expect(agent).toBeNull();
  });
