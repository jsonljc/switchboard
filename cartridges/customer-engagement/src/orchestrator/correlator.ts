...
export function detectCorrelations(result: JourneyDiagnosticResult): JourneyFinding[] {
  ...
  const proposedStage = result.stageAnalysis.find((s) => s.stageId === "service_proposed");
  const acceptedStage = result.stageAnalysis.find((s) => s.stageId === "service_accepted");
  ...
  const totalContacts = result.totalContacts.current;
  if (totalContacts > 20) {
