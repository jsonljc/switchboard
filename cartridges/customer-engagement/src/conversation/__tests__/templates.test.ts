...
describe("Conversation Templates", () => {
  describe("bookingFlow", () => {
    ...
    it("declares required variables", () => {
      expect(bookingFlow.variables).toContain("contactName");
      expect(bookingFlow.variables).toContain("serviceType");
    });

    it("includes a booking action step", () => {
      const actionStep = bookingFlow.steps.find((s) => s.type === "action");
      expect(actionStep).toBeDefined();
      expect(actionStep?.actionType).toBe("customer-engagement.appointment.book");
    });
  });
