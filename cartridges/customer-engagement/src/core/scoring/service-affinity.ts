...
import type { ServiceType, ServiceAffinityInput, ServiceAffinityResult } from "../types.js";

/**
 * Base affinity matrix: source treatment → target treatment → affinity (0-1).
 * Built from industry co-treatment patterns.
 */
const AFFINITY_MATRIX: Partial<Record<ServiceType, Partial<Record<ServiceType, number>>>> = {
 ...
 */
const AGE_MODIFIERS: Partial<Record<string, Partial<Record<ServiceType, number>>>> = {
 ...
 *
 * When `customMatrix` is provided (from a business profile), uses that matrix
 * instead of the hardcoded AFFINITY_MATRIX. Falls back to defaults otherwise.
 */
export function computeServiceAffinity(
  input: ServiceAffinityInput,
  customMatrix?: Record<string, Record<string, number>>,
): ServiceAffinityResult {
  const matrix = customMatrix ?? AFFINITY_MATRIX;
  const baseAffinities = matrix[input.currentService] ?? {};
  ...
  for (const [treatment, baseScore] of Object.entries(baseAffinities)) {
    const tt = treatment as ServiceType;

    // Skip if already done recently
    if (input.previousServices.includes(tt)) continue;
    ...
    if (finalScore >= 0.2) {
      candidates.push({
        ...
        reason: buildReason(input.currentService, tt, ageMod, budgetMod),
      });
    }
  }
