...
const createOverlayJsonSchema = zodToJsonSchema(CreateRoleOverlayBodySchema, {
  target: "openApi3",
});
const updateOverlayJsonSchema = zodToJsonSchema(UpdateRoleOverlayBodySchema, {
  target: "openApi3",
});
...
export const identityRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get(
    "/specs/by-principal/:principalId",
    {
      schema: {
