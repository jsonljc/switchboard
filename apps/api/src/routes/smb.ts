    ...
    async (request, reply) => {
      ...
      if (tier !== "smb") {
        return reply.code(400).send({
          error:
            "Activity log is only available for SMB organizations. Use the audit API for enterprise orgs.",
          statusCode: 400,
        });
      }
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      return reply.code(200).send({
        tier: "enterprise",
        message:
          "Organization upgraded to enterprise tier. Configure identity specs and policies to use the full governance pipeline.",
      });
    },
  );
};
