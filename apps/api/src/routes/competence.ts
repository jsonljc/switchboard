...
export const competenceRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get("/records/:principalId/:actionType", {
    schema: { description: "Get a specific competence record.", tags: ["Competence"] },
  }, async (request, reply) => {
    const { principalId, actionType } = request.params as { principalId: string; actionType: string };
  ...
  app.put("/policies/:id", {
    ...
    if (body["actionTypePattern"] !== undefined) updateData["actionTypePattern"] = body["actionTypePattern"];
