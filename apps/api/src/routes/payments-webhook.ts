...
export const paymentsWebhookRoutes: FastifyPluginAsync = async (app) => {
  app.post("/payments/webhook", { config: { rawBody: true } }, async (request, reply) => {
    ...
    if (!result.ok) {
      ...
      // of money that already moved, never an outbound paid action. Emit a reconciliation
      // signal so the settled-but-unrecorded payment is recoverable. Any other (retryable)
      // error keeps the 500 so Stripe's redelivery can succeed once the cause clears.
      if (result.error.type === "entitlement_required") {
        ...
        return reply
          .code(200)
          .send({ received: true, skipped: true, reason: "entitlement_blocked_reconcile" });
      }
    ...
    if (parked || result.result.outcome !== "completed") {
      app.log.error(
        ...
        "payment.record_verified did not complete; returning 500 so Stripe redelivers the settled deposit",
      );
      return reply.code(500).send({ error: "Payment not durably recorded", statusCode: 500 });
    }
