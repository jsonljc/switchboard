...
export const auditRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get(
    "/verify",
    {
      schema: {
        description:
          "Verify the integrity of the audit hash chain. Processes entries in chunks to avoid OOM.",
        tags: ["Audit"],
        querystring: {
          type: "object",
          properties: {
    ...
    async (request, reply) => {
      ...
      const maxEntries = Math.min(parseInt(query.limit ?? "10000", 10) || 10000, 100000);
