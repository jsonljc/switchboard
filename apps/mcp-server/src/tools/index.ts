...
export const SIDE_EFFECT_TOOLS = new Set([...sideEffectToolDefinitions.map((t) => t.name)]);

/** Set of read-only tool names for dispatch routing. */
export const READ_TOOLS = new Set([...readToolDefinitions.map((t) => t.name)]);
 ...
 */
export const MANUAL_ACTION_TYPES = new Set([...Object.values(SIDE_EFFECT_ACTION_TYPE_MAP)]);
