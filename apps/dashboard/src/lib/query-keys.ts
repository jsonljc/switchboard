...
/**
 * Bare (non-tenant-scoped) query keys.
 *
 * @deprecated Use `scopedKeys(orgId)` via `useScopedQueryKeys()` so cache
 * entries can never be observed across orgs. This export is kept ONLY to keep
 * existing call sites compiling during the staged refactor (PR-3 Tasks 27–29)
 * and will be removed in the same PR once every consumer is migrated.
 */
