...
export function NavBar() {
  ...
  const businessName = orgData?.config?.name;
  ...
  return (
    <>
      ...
      <aside className="hidden md:flex md:flex-col md:w-60 md:border-r md:bg-background md:fixed md:inset-y-0 md:z-40">
        <div className="flex flex-col h-14 px-4 border-b justify-center">
          <Link href="/" className="font-semibold text-lg">
            Switchboard
          </Link>
          {businessName && (
            <span className="text-xs text-muted-foreground truncate -mt-0.5">{businessName}</span>
          )}
        </div>
