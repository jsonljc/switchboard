...
export function PrintedPortraitAvatar({
  ...
  // Alex/Riley always render a sprite (fall back to idle frames if a requested
  // state is missing); only Mira (def === null) shows the in-frame identity letter.
  const sprite = def
  ...
  const letterSize = Math.round(size * 0.4); // ~0.4 cap-height for the Mira placeholder letter
