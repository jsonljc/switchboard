...
| AU-1       | HIGH     | shipped       | `docs/superpowers/specs/2026-04-29-fix-launch-webhook-hardening-design.md` (PR: TBD)     | Replay protection verified via existing `(channel, messageId)` dedup at `apps/chat/src/routes/managed-webhook.ts`; new tests `whatsapp-replay.test.ts`, `instagram-replay.test.ts` |
| AU-2       | HIGH     | shipped       | `docs/superpowers/specs/2026-04-29-fix-launch-webhook-hardening-design.md` (PR: TBD)     | Telegram `verifyRequest` fails closed in production; constructor logs error (no throw); test `telegram-fail-closed.test.ts`                                                        |
