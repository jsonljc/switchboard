...
| 6   | Policy conflict resolution untested                | ✅ SHIPPED | PR #TBD. New `packages/core/src/__tests__/engine-policy-conflict.test.ts` locks in the deny-loop-break behavior at `policy-engine.ts:309-313` with 6 cases covering allow/deny ordering, priority interleaving, `require_approval`+deny, and a positive-control two-allows case. In-source invariant comment added so the contract is discoverable from the engine. Audit's path hint (`skill-runtime/__tests__/`) was incorrect — that directory tests `SkillRuntimePolicyResolver` (an unrelated module). |
...
3. **Risk #3** — Outcome dispatcher idempotency completeness. S (decide whether local guard is needed beyond Meta-side dedup).
4. **Risk #4a** — Chat approval response identity binding. M (depends on chat contact→principal mapping).
