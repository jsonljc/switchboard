...
| 5   | Rate limits not per-endpoint                       | ✅ SHIPPED | PR #TBD. Per-route `config.rateLimit` overrides on `POST /api/approvals/:id/respond` and `POST /api/execute` via `@fastify/rate-limit`. Env names use `_HTTP_` infix (`APPROVAL_HTTP_RATE_LIMIT_MAX/WINDOW_MS`, `EXECUTE_HTTP_RATE_LIMIT_MAX/WINDOW_MS`, default 300/60000) to avoid colliding with the existing `APPROVAL_RATE_LIMIT_MAX` (per-responder anti-rubber-stamping cap inside `PlatformLifecycle`). `SENSITIVE_PREFIXES` left untouched — different threat model. |
