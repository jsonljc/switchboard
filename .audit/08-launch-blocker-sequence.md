...
| 3   | Ad-optimizer outcome dispatcher idempotency        | ✅ SHIPPED | PR #327. Defensive fix on the dormant `OutcomeDispatcher`: `OutcomeEvent` now requires `occurredAt` (with optional `eventId` escape hatch + optional `bookingId` disambiguator), `CapiLike.dispatch` requires `eventId`, new `synthesizeOutcomeEventId` helper hashes `(contactId, kind, bookingId, occurredAt)` with `\x1F` delimiter so Inngest retries dedupe at Meta on activation. Active CAPI path (`MetaCAPIDispatcher`) was already idempotent via `event.eventId`.                                                                                                                                                                                                                 |
| 4   | Chat approval binding hash not verified            | ✅ SHIPPED | PR #305 (gateway interception, terminal branch)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 4a  | Follow-up: Chat Approval Response Identity Binding | 🟡 PARTIAL | PR #329: infrastructure shipped — `OperatorChannelBinding` schema + store, shared `respondToApproval` helper extracted from `apps/api/src/routes/approvals.ts` into `packages/core/src/approval/`, `handle-approval-response` rewritten to do binding lookup → role check → shared-helper call (fail-closed when unwired; channel-possession ≠ authority). `DASHBOARD_HANDOFF_MSG` removed. **Production wiring deferred:** `apps/chat/src/gateway/gateway-bridge.ts` does not pass `approvalResponseConfig`, so live chat returns `NOT_AUTHORIZED_MSG` until follow-up PR wires `workTraceStore` + `lifecycleService` + `platformLifecycle` from `apps/chat`. No `skipResponderAuth` flag. |
| 5   | Rate limits not per-endpoint                       | ✅ SHIPPED | PR #324. Per-route `config.rateLimit` overrides on `POST /api/approvals/:id/respond` and `POST /api/execute` via `@fastify/rate-limit`. Env names use `_HTTP_` infix (`APPROVAL_HTTP_RATE_LIMIT_MAX/WINDOW_MS`, `EXECUTE_HTTP_RATE_LIMIT_MAX/WINDOW_MS`, default 300/60000) to avoid colliding with the existing `APPROVAL_RATE_LIMIT_MAX` (per-responder anti-rubber-stamping cap inside `PlatformLifecycle`). `SENSITIVE_PREFIXES` left untouched — different threat model.                                                                                                                                                                                                               |
| 6   | Policy conflict resolution untested                | ✅ SHIPPED | PR #326. New `packages/core/src/__tests__/engine-policy-conflict.test.ts` locks in the deny-loop-break behavior at `policy-engine.ts:309-313` with 6 cases covering allow/deny ordering, priority interleaving, `require_approval`+deny, and a positive-control two-allows case. In-source invariant comment added so the contract is discoverable from the engine. Audit's path hint (`skill-runtime/__tests__/`) was incorrect — that directory tests `SkillRuntimePolicyResolver` (an unrelated module).                                                                                                                                                                                 |
...
3. **Risk #4a follow-up** — Wire `apps/chat/src/gateway/gateway-bridge.ts` to pass `approvalResponseConfig` (binding store + identity store + respondDeps). S effort once chat-side `workTraceStore`, `lifecycleService`, and `platformLifecycle` are bootstrapped (today they are API-only).
