...
- `__tests__/drift-guard.test.ts` — the **fixture-coverage drift guard** (cloned from
  ...
  `in_learning_phase`, `burn`, `breach_building`, `audience_offer_mismatch`, `insight`).
...
| audience-offer mismatch → watch            | covered-at-seam                  | `audience-offer-mismatch.jsonl`: CTR up + CPA up-significant (1.6x, sub-2x) with flat CPM surfaces the lone `audience_offer_mismatch` diagnosis as an informational watch instead of pure silence (D1-3). Suppressed whenever a stronger signal (add_creative/pause/breach_building/landing_page_drop hold/burn) already fired, so it is additive visibility, never a new action path. `competition_increase` (informational) and `account_level_issue` (low confidence; cpl===cpa double-count) fire on this seam but stay deliberately advisory-only — see the marker comments in `metric-diagnostician.ts`. |
...
| scale on unproven paid value → watch (A12) | covered-at-seam                  | `scale-paid-value-gate.jsonl`: the count-vs-value gate on the `scale` → reallocate money-move. A cheap-cost-per-lead `scale` rec demotes to a `scale_unproven_paid_value` watch unless the campaign has finite positive verified-paid (`type="purchased"`) value (`scale-unpaid-demotes-to-watch` / `scale-zero-paid-demotes-to-watch` → watch), and flows when paid value is proven (`scale-paid-flows` → scale). `scale-control-no-paid-gate` (no `paidValueGate`) proves the gate is opt-in / back-compat. A prerequisite for flipping `RILEY_REALLOCATE_SELF_EXECUTION_ENABLED`.                           |
