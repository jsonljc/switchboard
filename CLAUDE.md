...
- **Worktrees have a setup step.** After `git worktree add`, run `pnpm worktree:init` from the new worktree root. The script delegates env setup to `setup-env.sh`, kills stale dev-port listeners, and (if Postgres is reachable) runs `pnpm db:migrate` → `pnpm build` → `pnpm db:seed`. If Postgres is unreachable, run `pnpm local:setup` after starting it. See `scripts/worktree-init.sh`.
