# Switchboard — Master Codebase Documentation
...
Switchboard is a **TypeScript monorepo** using pnpm workspaces and Turborepo for build orchestration. It consists of 4 **shared packages**, 4 **applications**, and 6 **cartridges** (domain plugins).

### Dependency Layers (strict, enforced by ESLint + dependency-cruiser)

```
Layer 1: schemas         → No @switchboard/* imports (pure types/Zod schemas)
Layer 2: cartridge-sdk   → May import schemas only
Layer 3: core            → May import schemas + cartridge-sdk
Layer 4: db              → May import schemas + core (NEVER cartridges)
Layer 5: cartridges/*    → May import schemas + cartridge-sdk + core (NEVER db/apps/other cartridges)
...
│   ├── cartridge-sdk/     — Cartridge interface, helpers, test harness
│   ├── core/              — Orchestrator, policy engine, all governance logic
│   └── db/                — Prisma ORM, all database store implementations
├── cartridges/
│   ├── customer-engagement/  — Lead management, conversations, appointments, cadences
│   ├── digital-ads/          — Multi-platform ad management (Meta, Google, TikTok)
│   ├── crm/                  — Contact, deal, and activity management
│   ├── payments/             — Stripe-backed payment operations
│   ├── revenue-growth/       — Autonomous revenue optimization (Theory of Constraints)
│   └── quant-trading/        — Stock/crypto trading operations
├── apps/
│   ├── api/               — Fastify REST API server (port 3000)
│   ├── chat/              — Multi-channel chat server — Telegram, WhatsApp, Slack (port 3001)
│   ├── dashboard/         — Next.js admin dashboard (port 3002)
│   └── mcp-server/        — Model Context Protocol server for LLM tool use
├── skins/                 — Vertical deployment configs (JSON) — clinic, gym, generic, commerce
├── profiles/              — Per-business instance configs (JSON) — clinic-demo, gym-demo
└── .github/workflows/     — CI (typecheck, lint, test, architecture, security, docker)
```
...
**What it does:** Defines every data type in the system using [Zod](https://zod.dev/) schemas. Zod gives you both TypeScript types AND runtime validation from a single definition. This package has zero internal dependencies — everything else imports from it.
...
**What it does:** Defines the interface that every cartridge (domain plugin) must implement, plus builder utilities, validation tools, and a test harness.
...
**File:** `orchestrator/propose-pipeline.ts` (857 lines — the largest file in the codebase)
...
**File:** `engine/policy-engine.ts` (551 lines)
...
**What it does:** All database interactions via Prisma ORM, plus credential encryption and OAuth token management.

**Database:** PostgreSQL 16, 30+ models defined in `prisma/schema.prisma` (714 lines).
...
**What it does:** Manages the entire customer journey — from first contact through qualification, appointment booking, follow-up, retention, and win-back. This is the primary cartridge for the lead-bot mode.
...
**What it does:** The most feature-rich cartridge. Manages digital advertising across Meta, Google, and TikTok — from campaign creation through optimization, creative management, audience building, and performance analysis.
...
**What it does:** Manages contacts, deals, activities, and pipeline health. Designed to be the single source of truth for customer relationships.
...
**What it does:** Manages invoices, charges, refunds, subscriptions, and payment links through Stripe.
...
**What it does:** Implements a **Theory of Constraints-inspired cyclic diagnostic loop** that continuously identifies and addresses the single biggest bottleneck to revenue growth.
...
**What it does:** Stock and crypto trading with governance guardrails. All market orders are critical risk (irreversible), limit orders are high risk (cancellable before fill).
...
**What it does:** The authoritative governance spine. All actions flow through the `LifecycleOrchestrator`. Built on Fastify.

**Boot Sequence** (`app.ts`, 391 lines):
...
**What it does:** Processes messages from Telegram, WhatsApp, and Slack. Can operate in two modes: **operator mode** (business owner manages operations via chat) or **lead bot mode** (engages prospective customers through qualification/booking flows).
...
**What it does:** Next.js 15 App Router application for configuring the AI assistant and monitoring operations.
...
**What it does:** Exposes Switchboard capabilities as MCP tools for integration with LLM assistants (Claude, ChatGPT, etc.).
...
| Job            | What It Checks                                                       |
| -------------- | -------------------------------------------------------------------- |
| `setup`        | Install, Prisma generate, migrate, build (cached by SHA)             |
| `typecheck`    | Full TypeScript type checking                                        |
| `lint`         | ESLint + Prettier formatting                                         |
| `test`         | Vitest with coverage, upload artifacts, check for untested packages  |
| `secrets`      | Gitleaks — scans git history for hardcoded secrets                   |
| `security`     | `pnpm audit` — dependency CVE scanning (high severity)               |
| `architecture` | `arch-check.ts` + `dependency-cruiser` — validates import boundaries |
| `docker`       | Build all 4 Docker targets (verifies Dockerfile correctness)         |

All status checks required to pass before PR merge. Branch protection enforced for admins.
...
| Metric                     | Count                                                                              |
| -------------------------- | ---------------------------------------------------------------------------------- |
| TypeScript source files    | ~1,400+                                                                            |
| Packages                   | 4 (schemas, cartridge-sdk, core, db)                                               |
| Applications               | 4 (api, chat, dashboard, mcp-server)                                               |
| Cartridges                 | 6 (customer-engagement, digital-ads, crm, payments, revenue-growth, quant-trading) |
| Total cartridge actions    | ~100+                                                                              |
| Skins                      | 4 (clinic, gym, generic, commerce)                                                 |
| Business profiles          | 2 (clinic-demo, gym-demo)                                                          |
| Database models            | 30+                                                                                |
| API route modules          | 27                                                                                 |
| CI pipeline jobs           | 8                                                                                  |
| Docker build targets       | 4                                                                                  |
| Audit event types          | 37                                                                                 |
| Policy condition operators | 13                                                                                 |
