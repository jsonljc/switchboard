import type { ReceiptedBookingRevenueData, ReceiptedBookingView } from "@switchboard/schemas";

/**
 * Weekly receipted-booking REVENUE for the owner report (north star: count -> proven booked revenue).
 * Consumes the pre-assembled cohort `views` (one shared `listForCohort` snapshot assembled once by
 * period-rollup, the SAME projection the count/quality dimensions see), so the cohort never
