 ...
 */
export function smbCategorizeRisk(amount: number | null, reversible: boolean): RiskCategory {
  if (amount === null) {
 ...
 * - locked → always
 */
export function smbApprovalRequired(risk: RiskCategory, profile: GovernanceProfile): boolean {
  switch (profile) {
    case "observe":
      return false;
    case "guarded":
...
export function smbEvaluate(proposal: ActionProposal, ctx: SmbEvaluationContext): DecisionTrace {
  ...
  const rawScore = riskCategory === "low" ? 15 : riskCategory === "medium" ? 45 : 75;
