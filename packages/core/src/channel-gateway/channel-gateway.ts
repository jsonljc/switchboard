...
async function dispatchResponse(params: {
  ...
  if (response.ok) {
    ...
    if (response.result.outcome === "pending_approval") {
      ...
      // lead. Like the failed/consent branches, send ONLY a framework-generated
      // holding notice (no agent-authored text, so it may bypass the consent
      // gate) and persist a metadata-only marker so operators see the park.
      try {
        await conversationStore.addMessage(
          conversationId,
          "assistant",
