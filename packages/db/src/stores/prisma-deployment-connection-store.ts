...
export class PrismaDeploymentConnectionStore {
  ...
  /**
   * Org-scoped variant of findByDeploymentAndType: only returns the connection
   * when the deployment belongs to `organizationId`. Defense-in-depth so a
   * credential read is tenant-safe at the store layer even if a caller forgets
   * the route-level org check (mirrors the org-scoping on updateStatus/delete).
   */
