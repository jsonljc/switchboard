...
**Known near-term follow-ups (display-only, non-blocking — from the final whole-impl review):** (1) **UGC detail-page parity** — the `/mira` cockpit list is UGC-aware (the seam maps `ugcPhase`/`ugcPhaseOutputs`), but `/mira/creatives/[id]` reuses the marketplace `CreativeJobSummary` wire type (which omits `mode`/`ugcPhase`/`ugcPhaseOutputs`), so a completed UGC job shows "Draft completed" in the list yet "No draft clip yet" on detail. Continue/Stop still work (the `/approve` API reads the real `job.mode`); it degrades gracefully and inherits a pre-existing dashboard UGC-blindness. PR5 anticipated this ("use the seam helpers only if the wire shape aligns; else read directly"). Fix: widen `CreativeJobSummary` to carry the UGC fields, or back the detail page with the seam. (2) Mira metrics route returns 503 if `reportStores` is absent though Mira never reads it (cosmetic). (3) Continue's cost label is generic ("a real cost") before the `storyboard` stage, since `/estimate` returns null until then (by design).

**Merge reconciliation vs current `main` (from the architecture-fit review, verified 2026-05-29):** the branch and `main` changed **disjoint file sets** (no textual conflicts), but two semantic items need attention at/after merge — (1) **agent-panel MiraPanel placeholder.** `main` shipped agent-panel Phase 2; `apps/dashboard/src/components/agent-panel/mira-panel.tsx` is a static "Mira isn't set up yet · Coming soon" body. After merge, an org with Mira **enabled** sees a working `/mira` cockpit but a "Coming soon" Mira drill-in from Home — make `MiraPanel` enablement-aware (link to `/mira` when `OrgAgentEnablement{mira,enabled}`). (2) **Latent (inert today):** `main`'s `agent-panel/key-result.tsx` calls `useAgentMetrics(agentKey, "all")`, but the Mira metrics route only accepts `window:"week"` (would 400 if a future MiraPanel renders a Mira key-result; `main`'s hook falls back to week, and MiraPanel is static today). (3) `main` added a `WorkTrace` idempotency migration — Mira makes no schema change, so no conflict, but run `pnpm reset && pnpm db:migrate` after merge. **Governance bridge follow-up (pre-broad-enablement, not M1):** Mira surfaces a cost-bearing "Continue draft" that rides the existing `lifecycle`-class `/creative-jobs/:id/approve` route (Inngest-dispatched, not via `PlatformIngress`/`WorkTrace`). This is a doctrine-sanctioned lifecycle bridge reused unchanged (no new bypass; org-scoped, regression-locked) — acceptable for pilot M1, but migrate the approve continue/stop onto `PlatformIngress` before broad enablement.
