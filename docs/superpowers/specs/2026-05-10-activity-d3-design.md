...
| 11  | Store interface extension       | Add `AuditLedger.listForBrowse(filter: AuditLedgerBrowseFilter)` to the existing `AuditLedger` interface in `packages/core/src/audit/ledger.ts`. The filter shape (precise definition in §5.3) takes `{organizationId, eventTypes, actorType, entityType, entityId, after, before, cursor, limit}` — no `scope`, no `sort`/`direction` (scope→eventTypes mapping happens in core; sort is always `(timestamp DESC, id DESC)` in v1). Do **not** overload existing `query()` — that's used by `auditRoutes`, `work-trace-integrity`, and `recommendations/interfaces.ts`; behaviour change there is risky. Cursor encoding and trimming live in core (§5.2). |
| 12  | View-model + naming             | Page-ready `AuditEntryBrowseRow` projection (see §4.2). New types live in the **existing** `packages/schemas/src/audit.ts`. API never returns raw `AuditEntry`. **Hard invariant: response never carries `snapshot` values, never carries `evidencePointers[].storageRef` content, never carries `redactedFields[]` (the original field names are themselves PII-shaped) directly — only a count.** The drawer surfaces a redacted projection only.                                                                                                                                                                                                         |
...
| 14  | `actorLabel` derivation         | `${actorType}:${truncate(actorId, 8)}` mono-prefix in the table; full `actorType` + `actorId` in the drawer. **No agent-name resolution in v1** (deferred per §1.2).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 15  | `entityLabel` derivation        | `${entityType}:${truncate(entityId, 8)}` mono-prefix in the table; full pair in the drawer.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| 17  | `snapshot` redaction            | Drawer surfaces an **allowlisted** subset of `snapshot` key names plus a `redactedKeyCount` for the rest. Allowlist (§4.5): `id`, `kind`, `source`, `decisionId`, `recommendationId`, `agentKey`, `targetEntityType`, `targetEntityId`, `correlationId`, `traceId`. Everything else is rolled into the redacted count. **No snapshot values, ever.**                                                                                                                                                                                                                                                                                                        |
...
| 28  | Polling cadence                 | **None on /activity v1.** React Query default `staleTime: 30_000` to dedup tab-switch refetches; explicit no `refetchInterval`. (Compare `useAudit()`, which polls 30s; D3's list semantics are different.) Manual refresh = page reload. "Tail mode" deferred to D3.5.                                                                                                                                                                                                                                                                                                                                                                                     |
...
- Drawer never surfaces raw `snapshot` values, never surfaces `evidencePointers[].storageRef` content, never surfaces `redactedFields[]` content directly (only a count); only `snapshot` allowlisted-key names + redacted count, only evidence-pointer `(type, hashPrefix)` pairs.
...
That's **28 event types in `Operational`** (out of 47 total in `AuditEventTypeSchema`).
...
packages/core/src/audit/
  ├─ ledger.ts (modify — add `listForBrowse` to AuditLedger interface)
...
packages/db/src/stores/
  └─ prisma-audit-store.ts (modify — implement listForBrowse)
...
These are the keys that appear across audit emitters with non-PII semantics (identifiers, kinds, sources). Adding a key requires updating the allowlist constant; no per-emitter customisation in v1.
...
Prisma implementation in `packages/db/src/stores/prisma-audit-store.ts`:
...
async listForBrowse(filter: AuditLedgerBrowseFilter): Promise<AuditEntry[]> {
  ...
  return rows.map((r) => AuditEntrySchema.parse(toDomainAuditEntry(r)));
}
```

In-memory implementation (`packages/db/src/stores/in-memory-audit-store.ts` — new sibling to the existing in-memory implementations) mirrors the same predicate logic over an array. Used by both the core `list-entries.test.ts` and the api `api-activity.test.ts` (per `feedback_api_test_mocked_prisma.md` — CI has no Postgres).
