...
compose intent resolves the creative deployment. After PR-2 the intent is live but has zero
initiators (same posture every creative intent shipped with).
...
| Compose exceeds the 30s skill-intent platform timeout                               | Zero tools, single turn, pre-digested context, taste lines capped (12) and performance summary capped (6 lines); latency recorded per compose via the trace hook; risk re-checked at PR-3 review with real timings                    |
