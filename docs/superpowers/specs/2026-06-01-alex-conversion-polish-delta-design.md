...
## Amendments — post-implementation architecture review (2026-06-02)

Two corrections to **Delta D** surfaced by the pre-merge architecture review and accepted:

1. **Showed stat is an honest raw count, not a coverage ratio.** The originally-specced `showCoverage = showed / booked` is **cohort-mismatched** — the numerator counts *opportunities* whose `updatedAt` (≈ stage→`showed` transition) fell in the week; the denominator counts *bookings* whose `createdAt` fell in the week. Different entities over different timestamps → the ratio can exceed 100% (a lead booked last week but shown this week inflates only the numerator). **Shipped instead:** the operator-confirmed Showed **count** + the `board updated <date>` freshness hint + the unavailable ("—") path when the board was never updated — which already serve the interpretability intent (distinguish low show-through from a stale board) without an unsound ratio. `showCoverage` removed from `MetricsViewModel` + all mirrors. A *true* show-through **rate** needs same-cohort booking→`showed` linkage (i.e. the deferred stage-history work) — out of scope. (Implemented: commit `a64ca43f`.)

2. **The dashboard stats render is de-scoped to a follow-up.** Verified: the dashboard renders **no `stats` row at all** — it was dropped in a prior agent-panel redesign; `agent-panel/key-result.tsx` renders only the hero, and no component reads `vm.stats`/`cell.hint`. So the honest Showed stat is **backend-correct + API-serialized but currently unrendered** (the pre-existing Leads/Conversion/Spend cells were equally unrendered — not a regression). The metric is honest at the source for any consumer; **surfacing a Leads/Showed/Spend strip is a separate UI-surface decision** (where it lives) tracked as a Phase-A follow-up, not part of this backend-polish pass. Delta D's "dashboard cell render" line and the Phase-A plan's Task 2 Step 10 are superseded by this de-scope.
