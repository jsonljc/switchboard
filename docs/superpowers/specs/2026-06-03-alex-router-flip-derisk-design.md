...
- Add an **8th optional constructor arg** to `SkillExecutorImpl`, `executionTraceHook?: Pick<SkillHook, "afterSkill" | "onError">` (typed structurally so it cannot be confused with the governance `hooks` array). Build a `SkillHookContext` once in `execute()` (all fields available from `params`/`requestCtx`). Wrap the turn loop in a try/catch keyed on that context: on the success return, `await this.executionTraceHook?.afterSkill(hookCtx, result)` inside a `.catch(log-and-swallow)`; on a thrown error, `await this.executionTraceHook?.onError(hookCtx, err)` (swallow) then re-throw. Telemetry failure must never change the lead-visible response.
