...
- `running` is **exclusively** a claim state on the ingress path (today no persisted ingress trace is ever `running`), so fail-closed on `running` is unambiguous. `queued` (legitimate async workflow result) keeps its existing cached-return behavior.
