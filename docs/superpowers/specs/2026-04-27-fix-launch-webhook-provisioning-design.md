...
9. **v1 channel limit — one managed channel per organization per channel type.** The existing schema constraint `@@unique([organizationId, channel])` on `ManagedChannel` is **intentional for this branch**. For a 10-org controlled beta, an org may connect at most one WhatsApp phone number, one Telegram bot, one Slack workspace. Multi-number support is deferred to a future schema-migration branch. Reasoning: this is a launch-provisioning branch, not a schema-design branch — preserve DB-level integrity and ship the spine. **No migration in this branch.** (Confirmed by Task 1: `phoneNumberId` is encrypted in `Connection.credentials`, not a top-level field — supporting `(orgId, channel, phoneNumberId)` would require either a migration or runtime decrypt-and-compare with no DB safety net. Both deferred.)
10. **Task 7 target — lazy OrganizationConfig upsert, not setup.ts.** Task 1 confirmed `setup.ts` only creates a `DashboardUser` with an org id; `OrganizationConfig` is created lazily on first API access via upsert at `apps/api/src/routes/organizations.ts:37–50`. Seed Alex listing + deployment **at that lazy-upsert site**. This boundary covers pre-existing orgs (no backfill needed) and is where the org first becomes operationally visible to the dashboard. The provision-time upsert stays as a safety net.
...
| A4  | Re-provisioning the same channel type for the same org is idempotent; a second WhatsApp number is rejected with a clear limit message. | (a) Integration test: call provision twice for `(orgId, "whatsapp", phoneNumberId="P1")`, assert exactly one ManagedChannel row exists and the second call returns/updates the existing row idempotently. (b) Integration test: provision `(orgId, "whatsapp", "P1")` then attempt `(orgId, "whatsapp", "P2")`, assert the second call returns `status: "error"` with `statusDetail` naming the v1 one-number-per-org limit. (c) Component test: dashboard surfaces the limit message clearly. **Multi-number support is deferred** to a future schema-migration branch — see Decision 9. |
...
| A7  | Alex appears in marketplace listings for a new org with **no** channels.                                                               | Integration test against the lazy OrganizationConfig upsert in `apps/api/src/routes/organizations.ts:37–50`: first dashboard API access creates or confirms `OrganizationConfig`, Alex listing+deployment is present before any channel is provisioned, repeated access does not duplicate listing/deployment.                                                                                                                                                                                                                                                                            |
...
1. **Idempotency on retry + second-number rejection (A4).** v1 limit per Decision 9: one managed channel per `(organizationId, channel)`. The existing `@@unique([organizationId, channel])` constraint backs this naturally — re-provisioning the same channel type returns/updates the existing row; a second `phoneNumberId` request is rejected (either at the unique-constraint level or by an explicit precheck) with a clear `statusDetail` naming the limit. **No migration in this branch.** Multi-number support is a deferred schema-design exercise.
...
1. ~~Where exactly is "org creation" today?~~ **Resolved by Task 1:** `setup.ts` creates `DashboardUser`; `OrganizationConfig` is created lazily on first API access via upsert at `organizations.ts:37–50`. Alex seed lives at the lazy-upsert site (Decision 10).
2. ~~Is there a unique constraint or natural key for `phoneNumberId`?~~ **Resolved by Task 1:** `phoneNumberId` lives only inside `Connection.credentials` (encrypted). v1 keeps the existing `(orgId, channel)` unique and accepts the one-number-per-org limit (Decision 9).
...
- `packages/db/prisma/schema.prisma` — no schema changes in this branch. The existing `@@unique([organizationId, channel])` is preserved per Decision 9. Multi-number support is a separate, future migration branch.
