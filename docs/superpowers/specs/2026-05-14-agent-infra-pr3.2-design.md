...
**Write path.** When `trackPattern` is called from `compounding-service.ts` (gated on `attribution.tier !== "none"` per PR-3.1), after merging/creating the `DeploymentMemory` row, write a `DeploymentMemoryEvidence` row in the same transaction with the `bookingId` from `BookingAttribution` and the `attributionTier` it resolved at. If attribution is `none` (impossible given PR-3.1's gate, but defensive), skip the evidence write.
