...
**Non-goals / deferred:** configurable booking governance dial; `ContactMutex`/idempotency-key; `btree_gist` range-exclusion; operation-level eval oracle; multi-booking disambiguation beyond "soonest + optional service narrow"; the conversation-lifecycle snapshot hook (separate, needs `conversationThreadId` — out of scope); **per-resource booking capacity** — the overlap guard (create _and_ reschedule) is **org-wide / single-resource-per-org**, faithfully mirroring the prior Local-provider `SLOT_CONFLICT` guard. `Booking` has no `calendarId`/room column to discriminate on, so capacity-aware overlap (parallel chairs/practitioners) is a follow-up requiring a schema change; org-wide is the correct, double-book-safe v1 default for a single-resource pilot.
