...
3. **Governance still runs on the child.** Verified: `PlatformIngress` applies the _identical_ pipeline (idempotency → entitlement → `validateTrigger` → `GovernanceGate`) regardless of `actor.type`/`trigger`. There is **no agent auto-approve bypass**, and the engine is **default-deny**. If a cost-bearing target were ever allowlisted, `approvalPolicy:"threshold"` + `budgetClass:"expensive"` makes it **park for human approval** — agent proposes, human disposes. (`creative.job.submit.allowedTriggers` would also have to be changed to include `"internal"`, a deliberate, reviewable act.)
