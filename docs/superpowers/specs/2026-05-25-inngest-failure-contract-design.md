...
**Consumes:** `docs/superpowers/specs/archive/2026-05-16-route-governance-contract-v1.md` (Route Governance Contract v1) §4, §9, §13.
...
A single typed shape for every async failure, declared in `@switchboard/schemas`. It extends WorkTrace's `ExecutionError` (`packages/core/src/platform/types.ts` — `{ code, message, stage? }`) so both contracts share the `{ code, message }` core that Route Governance Contract §13 names as the seam, and diverge only on async-specific metadata.
...
| `ad-optimizer-daily-signal-health`   | `packages/ad-optimizer/src/inngest-functions.ts`          | **A** | Gates recommendations; silent degradation misleads spend                                       |
...
| New `AuditEventType` member breaks exhaustive switches                      | Grep consumers of `AuditEventType` for exhaustive `switch` statements before adding the member; add the new arm where required.                                                                                                             |
