...
**The predicate returns a reasoned verdict, not a boolean** (rollout observability): `deriveCorroboration(input)` returns `{ causalStrengthUpgrade: "corroborated" | null, reason }` where `reason` is one of `corroborated | not_pause | visibility_flagged | missing_delta | unfavorable_direction | unstable_context | missing_booking_stats | missing_account_spend | non_finite_input | sparse_bookings | spend_continuity_failed | invalid_booked_value | ratio_degraded`. Tests pin exact reasons per failure mode, and debugging a rollout never requires reconstructing why corroboration stayed off. `attributeOneRecommendation` consumes only `causalStrengthUpgrade`; the persisted row stores only `causalStrength` (no reason column, no summary breakdown in v1; per-row reason logging is a recorded follow-on if rollout needs it).

**The finite guard (review-caught fabrication path).** Every reject gate in the predicate is a numeric comparison, and every comparison with NaN is false: without an explicit guard, a single malformed upstream value (Meta returns spend as strings; `parseFloat` of a non-numeric sentinel is NaN, and one NaN row poisons the account-wide sum) would sail past every floor to a fabricated `corroborated`. Defense in both layers: the predicate rejects any non-finite numeric input with `non_finite_input` immediately after the presence checks (the load-bearing fix, self-contained in core), and the adapter reports a non-finite account sum as ABSENCE (`accountSpendCents` omitted, predicate says `missing_account_spend`) rather than emitting the poison. Both pinned (predicate NaN/Infinity matrix; adapter NaN-row test).

**Persisted-contract note (review-surfaced, accepted deliberately):** once the live adapter populates `WindowMetrics.accountSpendCents`, every outcome row's persisted `metricSummary.preWindow`/`postWindow` JSON carries the key, for ALL kinds, corroborated or not. This is intentional: the row preserves the exact denominator evidence its causal-strength verdict was judged against (auditability), and no read model exposes `metricSummary` (grep-verified: zero shape-sensitive consumers). Pinned by an explicit row-level test so the contract is stated, not silent.
