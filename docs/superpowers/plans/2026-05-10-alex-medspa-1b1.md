...
- Persistent / cross-instance `ModeCache` (per-process is sufficient for the pilot)
...
- **Single shared `ModeCache` instance across both gates.** Wired at bootstrap (Task 14) and injected into both `DeterministicSafetyGateHook` and `ChannelGateway`. A warm hit by either gate warms the other.
...
If `DATABASE_URL` is unset / Postgres is not reachable, document this in the PR body and skip 3.4. The schema change still ships and `pnpm db:generate` validates the model declaration; the migration SQL must be regenerated by someone with DB access before deploy. Note this prominently in the PR description.
...
**Files:**
- Create: `packages/core/src/governance/mode-cache.ts`
- Create: `packages/core/src/governance/__tests__/mode-cache.test.ts`

**Why:** Per-process last-known-mode store. Shared between both gates so a warm hit by either warms the other. Constrains fail-closed behavior to deployments already known to be governed.
...
```typescript
import { describe, it, expect } from "vitest";
import { InMemoryModeCache } from "../mode-cache.js";

describe("InMemoryModeCache", () => {
  it("returns undefined for an unknown deploymentId", () => {
    const cache = new InMemoryModeCache();
    expect(cache.lastKnown("dep-1")).toBeUndefined();
  });

  it("returns the most recent remembered mode", () => {
    const cache = new InMemoryModeCache();
    cache.remember("dep-1", "observe");
    cache.remember("dep-1", "enforce");
    expect(cache.lastKnown("dep-1")).toBe("enforce");
  });

  it("isolates deployments from each other", () => {
    const cache = new InMemoryModeCache();
    cache.remember("dep-a", "enforce");
    cache.remember("dep-b", "off");
    expect(cache.lastKnown("dep-a")).toBe("enforce");
    expect(cache.lastKnown("dep-b")).toBe("off");
  });
});
```
...
```bash
pnpm --filter @switchboard/core test mode-cache
```
...
export class InMemoryModeCache implements ModeCache {
  private readonly store = new Map<string, GovernanceMode>();

  remember(deploymentId: string, mode: GovernanceMode): void {
    this.store.set(deploymentId, mode);
  }
...
```bash
git add packages/core/src/governance/mode-cache.ts \
        packages/core/src/governance/__tests__/mode-cache.test.ts
git commit -m "$(cat <<'EOF'
feat(core): InMemoryModeCache for governance config resolution fail-safe

Per-process Map<deploymentId, GovernanceMode>. Shared between
DeterministicSafetyGateHook and ChannelGateway pre-input gate so a
warm hit by either gate warms the other. Cache reset on process restart
is acceptable — first request after restart for an unwarmed enforce
deployment is the documented edge case (spec §1.5).
...
Create `packages/core/src/skill-runtime/hooks/__tests__/deterministic-safety-gate.test.ts`. The skeleton below uses placeholder names for the existing `AfterSkillContext` / output shape — replace them after Step 12.1 with the real names.

```typescript
import { describe, it, expect, vi, beforeEach } from "vitest";
import { DeterministicSafetyGateHook } from "../deterministic-safety-gate.js";
import { InMemoryModeCache } from "../../../governance/mode-cache.js";
...
// Replace the next two with the real codebase types after Step 12.1.
type AfterSkillContext = {
  deploymentId: string;
  sessionId: string;
  conversationId: string;
  skillOutput: { messages: { text: string }[] };
};
...
const buildDeps = (overrides: Partial<{
  ...
  return {
    deps: {
      ...
      modeCache: new InMemoryModeCache(),
...
describe("DeterministicSafetyGateHook.afterSkill", () => {
  ...
  it("fail-closed on resolver error with cache lastKnown=enforce", async () => {
    const cache = new InMemoryModeCache();
    cache.remember("dep-1", "enforce");
    const { deps, spies } = buildDeps({
      resolver: async () => ({ status: "error", error: new Error("db blip") }),
    });
    const hook = new DeterministicSafetyGateHook({ ...deps, modeCache: cache });
    const out = await hook.afterSkill(ctx("This is guaranteed.") as never);
    expect(out.skillOutput.messages[0].text).toContain("clinic team");
    expect(spies.verdictStore.save).toHaveBeenCalledTimes(1);
    expect(spies.verdictStore.save.mock.calls[0][0].reasonCode).toBe("governance_unavailable");
  });
...
Create `packages/core/src/skill-runtime/hooks/deterministic-safety-gate.ts`. Replace `SkillHook`, `AfterSkillContext`, `AfterSkillOutcome`, and any output-shape names with the real ones discovered in 12.1.
...
import type { ModeCache } from "../../governance/mode-cache.js";

// Placeholder shapes — replace with the real ones from skill-runtime/types.ts.
interface AfterSkillContext {
  deploymentId: string;
  sessionId: string;
  conversationId: string;
  skillOutput: { messages: { text: string }[] };
}
interface AfterSkillOutcome {
  skillOutput: AfterSkillContext["skillOutput"];
}
interface SkillHook {
  afterSkill(ctx: AfterSkillContext): Promise<AfterSkillOutcome>;
}

interface HandoffStoreLike {
  save(input: { reason: "compliance_concern"; payload: Record<string, unknown> }): Promise<void>;
}
interface ConversationStoreLike {
  setConversationStatus(sessionId: string, status: "human_override"): Promise<void>;
}
...
export class DeterministicSafetyGateHook implements SkillHook {
  ...
  async afterSkill(ctx: AfterSkillContext): Promise<AfterSkillOutcome> {
    ...
    if (resolution.status === "resolved") {
      ...
      this.deps.modeCache.remember(ctx.deploymentId, mode);
      if (mode === "off") {
        return { skillOutput: ctx.skillOutput };
      }
      ...
      const lastKnown = this.deps.modeCache.lastKnown(ctx.deploymentId);
      console.error(
        `[deterministic-safety-gate] resolver error for ${ctx.deploymentId}; lastKnown=${lastKnown ?? "none"}`,
        resolution.error,
      );
      if (lastKnown !== "enforce") {
        return { skillOutput: ctx.skillOutput };
      }
      // Fail-closed branch — synthesize a verdict and replace output.
      return await this.failClosed(ctx, "governance_unavailable", "SG", "medical");
    }
...
> **Note for the executing engineer:** The fail-closed default jurisdiction (`"SG"` / `"medical"`) is a placeholder for the case where the resolver errors and we have no parsed `GovernanceConfig`. If you want to remember the *config* in addition to the *mode*, extend `ModeCache` to a tuple cache (`{ mode, jurisdiction, clinicType }`) and use the cached values in `failClosed`. The spec accepts SG/medical as a safe default since the fail-closed branch only fires for deployments already known to be enforce-governed (and SG defaults are conservative). If your pilot is MY, swap the defaults at construction time via a `failClosedDefaults: { jurisdiction, clinicType }` dep on the hook.
...
cold-cache resolver error; fail-closed only with cache lastKnown=enforce
(reasonCode=governance_unavailable). Persistence failures do not skip
the block — emission integrity > persistence completeness.
...
**Why:** Mirrors the output gate's logic but inline in the gateway because it must short-circuit `platformIngress.submit()`. Shares the `ModeCache` instance with the output gate.
...
const buildGateway = (overrides: Partial<{
  ...
  const gateway = new ChannelGateway({
    ...
    verdictStore: verdictStore as never,
    handoffStore: handoffStore as never,
    modeCache: overrides.cache ?? new InMemoryModeCache(),
  } as never);
...
describe("ChannelGateway pre-input deterministic gate", () => {
  ...
  it("fail-closed on resolver error with cache lastKnown=enforce", async () => {
    ...
    expect(submit).not.toHaveBeenCalled();
    expect(send).toHaveBeenCalledTimes(1);
    expect(verdictStore.save.mock.calls[0][0].reasonCode).toBe("governance_unavailable");
  });
});
```
...
private async runDeterministicPreInputGate(args: {
  ...
  if (resolution.status === "resolved") {
    mode = resolveGovernanceMode(resolution.config);
    this.modeCache.remember(args.deploymentId, mode);
    if (mode === "off") return "proceed";
    jurisdiction = resolution.config.jurisdiction;
    clinicType = resolution.config.clinicType;
  } else {
    const lastKnown = this.modeCache.lastKnown(args.deploymentId);
    console.error(
      `[channel-gateway/pre-input-gate] resolver error for ${args.deploymentId}; lastKnown=${lastKnown ?? "none"}`,
      resolution.error,
    );
    if (lastKnown !== "enforce") return "proceed";
...
no-op. Resolver fail-closed only with cache lastKnown=enforce. Shares
ModeCache instance with the pre-output gate.
...
```typescript
import { InMemoryModeCache } from "@switchboard/core";
...
const modeCache = new InMemoryModeCache();
...
Shared InMemoryModeCache + PrismaGovernanceVerdictStore +
...
- ModeCache (per-process) — fail-closed only when last-known mode was enforce
...
- Persistent / cross-instance ModeCache
...
| §1.5 Resolver shape + ModeCache + decision rules | Tasks 10, 11, and applied in 12 & 13 |
...
| Open question 1 (persistent ModeCache) | Documented in Task 10 commit message; not implemented |
