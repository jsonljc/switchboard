...
| `apps/dashboard/src/lib/cockpit/activity-kind-map.ts`                              | Exports `translatedActionToActivityRow`; only consumer is its own colocated test. The active activity-stream path uses `useAgentActivityCockpit` (different hook) → `/api/dashboard/agents/[agentId]/activity` (returns `ActivityRow[]` server-side); `activity-kind-map.ts` is a vestige of an earlier client-side translation step that the dashboard proxy now performs server-side. | ✅ Verified: `rg "translatedActionToActivityRow" apps/ packages/` returns only the file itself and its test (2026-05-15).        |
| `apps/dashboard/src/lib/cockpit/__tests__/activity-kind-map.test.ts`               | Colocated test.                                                                                                                                                                                                                                                                                                                                                                         | ✅ Verified.                                                                                                                     |
...
- **Cockpit-side helpers that the A.5 brief speculatively listed for A.6 deletion but which are in fact active cockpit dependencies:**
  ...
  - `apps/dashboard/src/hooks/use-agent-activity.ts` — imported by `apps/dashboard/src/hooks/use-riley-activity.ts`, `apps/dashboard/src/hooks/use-riley-status.ts`, and `apps/dashboard/src/lib/cockpit/riley/riley-activity-translator.ts` (`TranslatedAction` type) and `apps/dashboard/src/lib/cockpit/riley/__fixtures__/riley-activity-fixtures.ts`. Active Riley-side dependency; **NOT a deletion candidate** (note: `cockpit-page.tsx` consumes `useAgentActivityCockpit` from a different file, `use-agent-activity-cockpit.ts`, which is unaffected).
    These two correct part of the A.5 brief's "What comes after A.5" sketch ([[alex-cockpit-a5-shipped]]). The third item in that sketch — `activity-kind-map.ts` — **is** an A.6 deletion candidate (verified orphaned; see §"Files deleted" table). See §Spec-conflict resolution.
...
- ❌ **No `legacy-shapes.ts` or `use-agent-activity.ts` deletion.** The A.5 brief sketched both for A.6 ([[alex-cockpit-a5-shipped]] "What comes after A.5"); the codebase shows them as active cockpit/Riley dependencies. The third file in that sketch — `activity-kind-map.ts` — IS deleted (verified orphaned). See §Spec-conflict resolution and §Design Decisions §4.
...
4. **`legacy-shapes.ts` and `use-agent-activity.ts` are NOT deleted at A.6; `activity-kind-map.ts` IS deleted.** The A.5 brief's "What comes after A.5" speculative sketch ([[alex-cockpit-a5-shipped]]) listed all three. Codebase verification (2026-05-15) shows two of them as active cockpit dependencies: `legacy-shapes.ts` is imported by `kpi-strip.tsx` + `metrics-to-kpi-input.ts`; `use-agent-activity.ts` is imported by Riley-side hooks + translator. **But `activity-kind-map.ts` is orphaned** — `rg "translatedActionToActivityRow" apps/ packages/` returns only the file itself and its own test. The active activity-stream path uses `useAgentActivityCockpit` (a different hook at `use-agent-activity-cockpit.ts`) which calls the `/api/dashboard/agents/[agentId]/activity` endpoint and receives `ActivityRow[]` from the server. `activity-kind-map.ts` is a vestige of an earlier client-side translation step that the dashboard proxy now performs server-side. Per the user's prompt: "Resolve in favor of the narrowest possible cleanup interpretation" — narrowest still deletes provably-orphaned dead code.
...
4. **A.5 brief's "What comes after A.5" speculative sketch lists `legacy-shapes.ts`, `activity-kind-map.ts`, and `use-agent-activity.ts` for A.6 deletion.** Codebase verification (2026-05-15) shows two of them (`legacy-shapes.ts`, `use-agent-activity.ts`) as active cockpit/Riley dependencies and one (`activity-kind-map.ts`) as genuinely orphaned. **Resolution:** brief §Design Decisions §4 overrides the sketch partially — `legacy-shapes.ts` + `use-agent-activity.ts` are NOT deletion candidates; `activity-kind-map.ts` IS a deletion candidate (added to §"Files deleted" with its colocated test).
