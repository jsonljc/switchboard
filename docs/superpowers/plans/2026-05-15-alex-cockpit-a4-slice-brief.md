...
A.4 ships **no schema change**. The `ActivityRow` type already declares the full superset at `apps/dashboard/src/components/cockpit/types.ts:102-111`. The `ConversationMessage` Prisma model at `packages/db/prisma/schema.prisma:861-873` already carries the `(contactId, orgId, direction, content, createdAt)` tuple the preview reader needs. No new Zod schemas, no new Prisma columns, no migration, no `db:check-drift` concern.

### Core (surface-agnostic helpers)

| Path | Change | Why touched |
|---|---|---|
| `packages/core/src/agent-home/activity-preview-reader.ts` | **New.** `interface ActivityPreviewReader { readRecent(args: { contactId: string; orgId: string; limit: number }): Promise<Array<{ from: "contact" \| "alex" \| "operator"; text: string; createdAt: string }>> }` | Layer 3 surface-agnostic contract. Consumed by the API translator. No UI references. |
...
| `packages/core/src/agent-home/cockpit-activity-translator.ts` | **New.** `translateAuditToCockpitActivity(args: { entries: AuditEntry[]; previewReader: ActivityPreviewReader; orgId: string; limit: number; expandPreview: boolean }): Promise<ActivityRow[]>` — pure(-ish) translator that consumes audit entries, derives `kind` (via the existing kind-classifier), extracts `who` / `contactId` from snapshot, populates `body` from snapshot-specific copy templates, and (when `expandPreview`) batches a single `previewReader.readRecent` call per unique contact. | Layer 3 server-side translator. |
| `packages/core/src/agent-home/__tests__/cockpit-activity-translator.test.ts` | **New.** Per-kind translation cases + batched-preview-fetch case + expandPreview=false short-circuit case + missing-contactId fallback case. | Coverage. |
...
The translator emits the dashboard's `ActivityRow` shape directly. Since `ActivityRow` is currently declared in `apps/dashboard/src/components/cockpit/types.ts` (Layer 5), A.4 **does not move the type into core** — instead, core declares its own structurally-identical `CockpitActivityRow` type and the dashboard treats them as parallel wire shapes. The mirror is enforced by a convention test in the API package that imports both and asserts shape-equivalence at compile time (`type _Mirror = CockpitActivityRow extends ActivityRow ? true : never;` + `const _: _Mirror = true;`).

This avoids cross-layer leakage (`apps/dashboard` types are not consumed by `packages/core`) while keeping a single authoritative definition for the runtime translator.

### DB

| Path | Change | Why touched |
|---|---|---|
| `packages/db/src/prisma-activity-preview-reader.ts` | **New.** `class PrismaActivityPreviewReader implements ActivityPreviewReader` — wraps `prisma.conversationMessage.findMany({ where: { contactId, orgId }, orderBy: { createdAt: "desc" }, take: limit, select: { content, direction, createdAt, metadata } })` and maps `direction` → `from`. **Critically batched at the translator boundary, not here** — this reader is per-contact; the translator caller groups by contact and dispatches one call per unique contact (or, in the canonical implementation, the reader exposes a sibling `readRecentBatch({ contactIds, orgId, limit })` that issues one `findMany({ contactId: { in: contactIds } })` and groups in memory). | Layer 4 Prisma impl. |
...
| Path | Change | Why touched |
|---|---|---|
| `apps/api/src/routes/agent-home/activity.ts` | **New.** `GET /agents/:agentId/activity` Fastify route. Reads `limit` (default 50, max 200) + `expandPreview` (default `true`) query params. Resolves the audit ledger via `request.auditLedger` (or org-scoped Prisma query), invokes `translateAuditToCockpitActivity` with a `PrismaActivityPreviewReader`, returns `{ rows: CockpitActivityRow[] }`. | Endpoint. |
...
| `apps/api/src/lib/activity-translator-deps.ts` | **New.** Factory that wires `PrismaActivityPreviewReader` + the audit query function and returns the bound translator dependency object. Mirrors `apps/api/src/lib/meta-spend-provider.ts` (A.3's wiring helper). | Dependency injection. |
...
| `apps/dashboard/src/components/cockpit/types.ts:111` | Extend. | `ActivityRow` has 8 fields. | Add `contactId?: string;` (drives the `/contacts/[contactId]?note=open` deep link). Optional — when undefined, the affordance is hidden. |
...
Per `feedback_surface_agnostic_backend.md`: core/schemas/db must not reference UI surfaces. The new translator lives in `packages/core/src/agent-home/` (Layer 3) and consumes only audit-domain types + the `ActivityPreviewReader` interface. The Prisma reader lives in `packages/db/src/` (Layer 4) and depends only on `@switchboard/core`. Neither package references `apps/dashboard`. `CockpitActivityRow` is declared in core and structurally mirrors `apps/dashboard/.../types.ts:ActivityRow`; the compile-time mirror assertion sits in the API package (Layer 5), where cross-layer references are allowed.
...
2. **`ActivityRow.contactId` is the new optional field, not a new schema layer.** `contactId` is declared in `apps/dashboard/src/components/cockpit/types.ts` (the wire shape between proxy and component). It is **not** a Zod schema field at any layer because the wire shape is not Zod-validated today (the route proxies a typed Fastify response). If a future need requires schema validation, the type lifts into `packages/schemas/src/cockpit-activity.ts` with a discriminated union per kind — but A.4 does not pay that cost.
...
4. **Activity-row stable key.** Today's stream uses `${row.time}-${row.head}-${i}` as the React key, which breaks when the same head is emitted twice with the same time. **Mitigation:** A.4 adds an optional `id: string` field to the wire shape (`auditEntry.id` passed through). The component falls back to the legacy key when `id` is missing.
...
- **Convention test:** `packages/core/src/agent-home/__tests__/cockpit-activity-row-mirror.test.ts` asserts shape-equivalence between `CockpitActivityRow` (core) and `ActivityRow` (dashboard) at compile time via `type _Mirror = CockpitActivityRow extends ActivityRow ? true : never; const _: _Mirror = true;`. The test file lives in core and imports the dashboard type via `import type { ActivityRow } from "@switchboard/dashboard-types"` **only if** a dashboard-types package exists; otherwise the assertion lives in the API package (Layer 5) where cross-layer imports are permitted. Plan picks the latter to avoid creating a new package.
