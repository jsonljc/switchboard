...
- [ ] **Step 3: Switch to non-blocking index creation**

Edit `$DIR/migration.sql` so the index creation is non-blocking. Replace the generated `CREATE INDEX "Booking_organizationId_createdAt_idx" ON "Booking"(...)` with:

```sql
CREATE INDEX CONCURRENTLY IF NOT EXISTS "Booking_organizationId_createdAt_idx"
  ON "Booking"("organizationId", "createdAt");
```

Prisma's migration runner requires `CREATE INDEX CONCURRENTLY` to run outside a transaction. Add this comment as the first line of the file (Prisma honors this directive):
...
If your installed Prisma version does not honor that comment, fall back to plain `CREATE INDEX` and accept the brief lock — but verify in step 4 by inspecting the generated SQL.
...
These are stubs — Tasks 5 and 6 replace their bodies with real logic.
...
- [ ] **Step 1: Inspect the wins isolation test for the buildTestServer signature**

```bash
head -50 apps/api/src/__tests__/api-agent-home-wins-isolation.test.ts
```

Note the imports, fixture-row structure, and the way org A vs org B requests are issued.
...
describe("metrics route — cross-tenant isolation", () => {
  it("org A request never reads org B rows (and vice versa)", async () => {
    const calls: { orgId: string; method: string }[] = [];

    const bookingStore = {
      countExcludingStatuses: vi.fn(async ({ orgId }: { orgId: string }) => {
        calls.push({ orgId, method: "bookings" });
        return orgId === ORG_A ? 7 : 99;
      }),
    };
    const conversionRecordStore = {
      countByType: vi.fn(async (orgId: string) => {
        calls.push({ orgId, method: "conversions" });
        return orgId === ORG_A ? 13 : 200;
      }),
    };
    ...
    const orgACalls = calls.filter((c) => c.orgId === ORG_A);
    const orgBCalls = calls.filter((c) => c.orgId === ORG_B);
...
If `buildTestServer` does not accept a `reportStores` override, mirror exactly how the wins isolation test injects its `recommendationStore` and adapt accordingly. Read the existing test-server signature before adapting.
