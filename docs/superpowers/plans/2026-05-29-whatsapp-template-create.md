...
**Architecture:** Four isolated layers mirroring the existing send-test slice — a Zod request schema (`packages/schemas`), a mutating Fastify route reusing `graphPost`/`isRetryable` (`apps/api`), a react-query mutation hook that refetches the template list, and a `Dialog` form wired into the existing Templates card. A throwaway seed (uncommitted) lights up the page. The `/api/dashboard/*` → Fastify forwarding is handled generically by `apps/dashboard/src/middleware.ts`, so **no Next route handler is needed**.
