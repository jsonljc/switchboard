...
- **Riley `<Composer>` adoption** — Riley currently renders `<ComposerPlaceholder>` (the inert A.1 bar) and B.3-followup may or may not adopt the new `<Composer>` at that time. A.5 writes `<Composer>` to be agent-agnostic at the prop level: it takes `placeholder`, `onDispatch`, and `halted` — and it does not own toast behavior or command-id routing. Riley adoption requires a Riley-side dispatcher hook (`useRileyActionDispatcher`) wired into `onDispatch`, not Composer-level `toastVoice` / `commandIdHandler` props.
...
|---|---|---|---|
| `apps/dashboard/src/components/cockpit/command-palette.tsx` | **New.** | n/a | `<CommandPalette open onClose commands onSelect threadContext? placeholder?>` — modal-style palette anchored under the Topbar. Renders a search input + grouped command list (order: `control` → `thread` → `rules` → `nav`). The palette's primary job is "tell Alex what to do," so operational commands (control + thread) lead; nav is last because it routes away from the cockpit. Thread-group commands render disabled when `threadContext` is `undefined`. Up/down arrows move selection; Enter fires `onSelect(command)`; Escape calls `onClose()`. **No agent-specific logic.** |
...
| `apps/dashboard/src/components/cockpit/cockpit-page.tsx:8-25,94,135` | Modify. | Imports `ComposerPlaceholder`; passes `paletteEnabled={false}` to Topbar; renders `<ComposerPlaceholder halted={...}>`. | Imports `Composer` + `CommandPalette` + `useAlexActionDispatcher` + `ALEX_COMMANDS` + `ALEX_COMPOSER_PLACEHOLDER`. (Does **not** import `toastVoice` — that lives inside the dispatcher.) Adds `paletteOpen` state via `useState<boolean>(false)`. Adds keyboard-shortcut effect listening for `⌘K` / `Ctrl+K` (page-scoped — listener mounts on `CockpitPage` mount only, unmounts on unmount, no global registration). Passes `paletteEnabled={true} onOpenPalette={() => setPaletteOpen(true)}` to Topbar. Renders `<CommandPalette open={paletteOpen} onClose={...} commands={ALEX_COMMANDS} onSelect={(cmd) => dispatch({ kind: "command", commandId: cmd.id }, threadCtx)} threadContext={...} />`. Renders `<Composer placeholder={ALEX_COMPOSER_PLACEHOLDER} onDispatch={dispatch} halted={haltCtx.halted} />` in place of `<ComposerPlaceholder>`. `ALEX_COMMANDS` is for `<CommandPalette>` only — Composer takes neither `commands` nor `toastVoice`. |
...
- ❌ **Typed `hold` action kind.** `hold-named` is catalog-visible (per the locked design) but A.5's `ParsedActionKind` discriminated union does **not** include a `hold` variant. Because `threadContext` is always undefined at the A.5 call site, `hold-named` is permanently disabled in the palette and never reaches the dispatcher in A.5 — so the missing kind is observably inert. A future thread-context slice owns the choice: either add `hold` to `ParsedActionKind` + the dispatch table (with a defined behavior, e.g. routing to `/contacts/[id]?hold=true`), or remove `hold-named` from the catalog. **A.5 does neither.** Recorded as a known catalog/dispatcher asymmetry; the implementation plan ships a guard comment + a unit assertion that the palette's filtering keeps it inert.
...
1. **NL parser is a verbatim port of `commands.jsx:7`, plus one word-quantifier extension to keep the locked placeholder honest.** Every pattern in the umbrella spec §Composer §NL parsing rules table (lines 670-685) lands in `parse-command.ts`. **Plus:** `pause (for) (an|one|half an) (hour\|hours\|min\|minute\|minutes)` resolves to the same shape as `pause for N(unit)` with `an|one → 1`, `half an → 0.5`. The locked placeholder string `'Tell Alex what to do — "pause an hour", "follow up with Maya tonight"…'` advertises `"pause an hour"`; the verbatim port of the JS regex does not parse that input, falling instead to the `instruction` fallback. The word-quantifier extension is the minimum change required to keep the placeholder example honest. Documented as a deliberate deviation from a strict port — not a spec amendment, because the placeholder string itself is normative and the parser must serve it.
...
3. **`<Composer>` is agent-agnostic at the prop layer.** Props: `{ placeholder, onDispatch, halted }`. The Composer does not call `useHalt` directly — the `halted` value is passed in, so the component can be rendered in tests without `<HaltProvider>`. **Critically, the Composer never calls `useToast` or `toastVoice`.** Its only responsibility is parsing input, staging the chip preview, and dispatching the parsed action. Toasting is the dispatcher hook's responsibility (Decision #4) — a single owner avoids the double-toast hazard that arises if both Composer and dispatcher fire on the same Enter press.

4. **Action-dispatch glue lives in a Dashboard hook, not in the Composer.** `useAlexActionDispatcher()` is the Alex-specific dispatcher: it owns the action.kind → side-effect mapping, the per-id overrides for `command` group, **and** the toast firing (via `useToast` + `toastVoice`). The Composer remains agent-agnostic by delegating dispatch to its `onDispatch` prop. Riley B.3-followup will ship `useRileyActionDispatcher()` with its own mapping; the Composer + Palette do not change. **The toast boundary is locked at exactly one owner:** the dispatcher hook. Composer, Palette, and CockpitPage never call `useToast` for composer-action paths.
