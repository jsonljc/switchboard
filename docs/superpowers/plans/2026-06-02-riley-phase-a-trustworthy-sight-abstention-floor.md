...
it("RecommendationOutput requires a resetsLearning class", () => {
  ...
  expect(RecommendationOutputSchema.safeParse({ ...base, resetsLearning: "no" }).success).toBe(
    true,
  );
});
```
...
In `campaign-decision.ts`, compute window evidence from the insight and pass it through. `days` = the breach window the provider used is 14, but the _current-period_ insight is the 7-day window; use the count of breach periods is not evidence — use the insight's own window. Set:
...
Verify each `expectedOutcome` by running `pnpm eval:riley` and adjust the fixture (not the code) if the reduced label differs — fixtures encode the _desired_ behavior this task implements; if a label is genuinely different (e.g. `insight` vs `watch` for the spike), set it to the real abstention label the pipeline produces and note why.
...
it("V2 lockout holds ANY reset-class action during learning, not just pause/restructure", () => {
  const v2 = new LearningPhaseGuardV2();
  const learning = v2.classifyState({
    /* LEARNING ad set input */
  });
  const refresh = {
    /* a refresh_creative RecommendationOutput, resetsLearning:"yes" */
  };
  expect(v2.gate(refresh, learning).type).toBe("watch"); // today this PASSES THROUGH (bug)
  const hold = {
    /* a hold RecommendationOutput, resetsLearning:"no" */
  };
