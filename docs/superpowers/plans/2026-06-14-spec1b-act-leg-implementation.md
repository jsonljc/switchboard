...
v1 also assumes a **2-decimal (hundredths) account currency** (the SG/MY pilot is SGD/MYR). `getAccountDailySpendCents` converts the major-unit insights `spend` to cents with `x100`, which is unit-consistent with `getCampaign`'s verbatim minor-unit `daily_budget` **only for hundredths currencies**. A zero-decimal currency (JPY/KRW, where the budget and spend share the same unit) would mis-scale the blast-radius share denominator `100x` against the verbatim budget - the unsafe direction (an understated share passes a move that is actually over the SHARE_CAP). So non-hundredths currencies are out of v1 scope; currency-aware spend conversion (read the account currency, scale by its minor-unit exponent) is a deferred fast-follow before any non-hundredths market. The 2-decimal assumption is documented at the `getAccountDailySpendCents` call boundary.
