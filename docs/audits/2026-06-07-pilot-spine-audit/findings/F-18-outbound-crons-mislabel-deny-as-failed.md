# F-18: Outbound crons record a governance DENY (and a park) as `no_terminal_outcome` "failed" — misleading label + wasted retry budget

- **Severity:** cosmetic/low (data-integrity + debuggability — no customer-visible harm, no false operator alerts; a denied reminder correctly was not sent; the mislabel and retry burn are a debuggability and budget waste concern)
...
- **Location (verified against `main`, worktree `audit/pilot-spine`, 2026-06-08):**
  ...
  - **No false operator alerts** — the `onFailure` / `alert:true` handler on the Inngest function fires on Inngest retry-exhaustion/throw (`async-failure-handler.ts:84-103`), NOT on a `markFailed` row written inside a normally-returning run. The crons swallow the governance deny and return counts — no Inngest-level failure is raised, no alert fires. A denied or parked cron tick completes normally from Inngest's perspective.
...
- Full code trace of the response contract from `platform-ingress.ts` through both dispatchers and `async-failure-handler.ts`. The live exercise (trigger the Inngest function, seed a confirmed booking, observe the gate deny + `markFailed`) was NOT run because no Inngest dev server is running locally and the API's `/api/inngest` is the serve handler, not an invoke endpoint (documented in D-05 and verdict map J7-S3). The code trace is definitive: there is exactly one fall-through for an `outputs.sent === undefined` response, and both deny and park produce that.

## What happened vs expected

- **Expected:** a governance DENY should be recorded as a denial/skip (a distinct, non-retrying outcome), and a require_approval park should be recognized (and ideally deferred/parked), not logged as a generic failure.
- **Observed:** both collapse to `no_terminal_outcome` → `markFailed`. Under the F-16 default-deny floor every reminder/follow-up for a fresh pilot org becomes a `failed` row with a misleading reason. No customer-visible harm (the reminder correctly was not sent). No false operator alerts (the `alert:true` / `onFailure` path is not triggered by an in-run `markFailed`). The follow-up cron additionally burns its 3-attempt retry budget re-submitting a permanent deny.
...
- In both dispatchers, branch on the response before the `outputs.sent` check: add `if ("approvalRequired" in response && response.approvalRequired)` → mark deferred/parked (not failed, no retry-budget burn); and detect a governance deny (e.g. `response.result.outcome === "failed" && response.result.error?.code` present, or thread a denial signal) → `markSkipped(id, "governance_denied")` with no retry burn. Reserve `no_terminal_outcome` "failed" for a genuinely malformed execution result.
