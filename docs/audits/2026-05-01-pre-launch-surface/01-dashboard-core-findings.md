...
Checked: D — see findings below (closeout state walk pending; DC-58 from spontaneous error-state observation)
...
In practice the cache is destroyed today by two facts: (1) `signOut({ callbackUrl: "/login" })` and (2) successful credentials sign-in (`window.location.href = callbackUrl` on login page) both perform full-page navigations that throw away the React tree and the QueryClient with it. So the _currently-shipping_ sign-out → sign-in flow does not leak. **But** the missing tenant scoping means any future change that:
...
**What:**
On `/console`, the page calls `useConsoleData()` _before_ checking `useSession().status`. While `status === "loading"`, `useConsoleData` returns the hardcoded `consoleFixture` (per DC-04). Combined with the absence of `/console` in the middleware matcher (DC-10), an unauthenticated visitor briefly sees the Aurora Dental fixture content paint before the client-side `redirect("/login")` fires once `status` resolves to `"unauthenticated"`. This is not a tenant-data leak (the fixture is static demo data), but it is an "auth boundary leaks UX state" issue: the page should be treated as gated, not as something that paints first and gates after.
...
**What:**
After a successful escalation reply, the card renders a blue info banner that reads: _"Your reply has been saved. It will be included in the conversation when the customer sends their next message. Direct message delivery is coming in a future update."_ This is factually false today — the API path only returns 200 OK when proactive channel delivery to Telegram/WhatsApp/Slack succeeds (`apps/api/src/routes/escalations.ts:266-275` returns 502 unless `deliveryResult === "delivered"`). The mutation only fires `setSent(true)` on a 200, so by the time this banner appears the customer has already received the message via their channel. Telling the operator the reply will _only_ arrive when the customer next messages — and that direct delivery is "coming in a future update" — risks the operator either re-sending the reply through another channel, or simply not trusting the system to deliver.
...
- Repro:
  ...
  4. On success (200 from `/api/dashboard/escalations/{id}/reply`), the blue info banner appears reading _"Your reply has been saved. It will be included in the conversation when the customer sends their next message. Direct message delivery is coming in a future update."_
...
**Fix:**
Replace the banner with a confirmation that matches reality: _"Reply sent to {customer} on {channel}."_ If channel delivery is degraded (e.g. notifier missing, 502 path), surface that as an explicit error state with retry guidance, not the generic "saved" banner.
...
**What:**
The empty state branches on `filter === "pending"` only. The else-branch covers both the _Released_ and _Resolved_ tabs and renders the literal string "No resolved escalations yet." When the operator clicks the _Released_ filter and finds it empty, the page tells them they have no resolved escalations — wrong noun, different lifecycle stage. The pill they just clicked says _Released_; the empty state should match it.
...
**Fix:**
Pick one verb (recommended: _Decline_, since the dialog explicitly tells the user the action will be cancelled) and use it across button, dialog, toast, and history. Or pick _Reject_ and ditch "Not now" / "Declined." Either choice is fine — the inconsistency is the issue.
...
**What:**
The detail page renders a labeled, copy-pasteable "Binding Hash" field with a monospaced multi-line block, and an "Approvers" list of opaque principal IDs in monospace badges. _Binding hash_ is an internal cryptographic primitive used for replay-prevention in the API request body (`packages/core` approval flow) — operators do not act on it, do not need to read it, and have nothing to do with it. _Approvers_ shown as raw principal IDs (e.g. `op-abc123`) is similarly internal. Both leak the audit/security plumbing into the operator's primary decision view, which makes the page look engineering-facing rather than owner-facing.
...
**What:**
When `currentIntent` is null, the conversation card renders the literal "No intent yet." _Intent_ is internal taxonomy from the agent runtime; an operator scanning a list of customer threads doesn't think of conversations as having an "intent." The empty-state copy reads engineering-y and offers no signal as to why the field is blank.
...
**Fix:**
Add `aria-hidden="true"` to each decorative Lucide icon. Lucide's React components accept the prop directly. Where the icon is the _only_ indicator (none of the cases above qualify, but a future icon-only chip would), instead add an accessible name via `aria-label` on the parent.
...
- **Surface:** /console
- **Sub-surface:** error fallback (whole-page render)
- **Dimension:** D, H
- **Severity:** Launch-blocker
- **Affects:** all users
- **Status:** Open
...
**What:**
On `/console`, when any of the five hooks (`useDashboardOverview`, `useEscalations`, `useOrgConfig`, `useModuleStatus`, `useAudit`) errors, the page renders **only** the "Couldn't load live data. Showing the last known shape." banner and a blank body — the `consoleFixture` fallback declared at `use-console-data.ts:56-58` does not visibly render. Operators landing on the highest-traffic post-launch surface during any backend hiccup see a near-empty page under a banner that lies about there being a "last known shape." Worse than DC-04 (which assumed the demo fixture would at least render).

**Evidence:**

- File: apps/dashboard/src/components/console/use-console-data.ts:56-58 (returns `consoleFixture` on any hook error — yet the page renders empty)
- File: apps/dashboard/src/app/(auth)/console/page.tsx:14-25 (page wraps banner + `<ConsoleView data={data} />` together; no conditional that should hide ConsoleView)
- File: apps/dashboard/src/components/console/console-view.tsx:158-162 (ConsoleView always emits `<div data-v6-console>` with header zones; no auth/state gating)
- Screenshot: artifacts/01-dashboard-core/screenshots/states/error-console-blank-body.png
- Repro:
  1. With dashboard running on :3002 and API on :3000, sign in to a tenant whose API endpoints return non-200 for `/api/dashboard/overview` (any tenant where org/session resolution fails — easiest repro: sign in to a brand-new account before org seeding completes).
  2. Navigate to `/console`.
  3. Observe: error banner renders top-of-page; body below the banner is blank. None of the opstrip / numbers strip / queue / agents / Nova panel / activity zones from `consoleFixture` appear, despite the code path returning the fixture on error.
  4. Browser network tab will show repeated 500s on `/api/dashboard/overview`, `/api/dashboard/modules/status`, `/api/dashboard/escalations`, `/api/dashboard/organizations`.

**Fix:**
Either (a) repair the fallback path so the fixture actually renders when error is set (root-cause likely a CSS / layout / Suspense boundary somewhere between `<OwnerShell>` and `<ConsoleView>` — needs a render-tree investigation), and rewrite the banner copy per DC-04 since "last known shape" is still false; OR (b) replace the fixture-fallback with a real empty/error state (skeleton or "We couldn't load your console — try again or contact support") so an empty page is impossible regardless of which hook fails. (b) is the right answer — fixture-as-fallback is the architectural mistake DC-04 already flagged.

---

## DC-59

- **Surface:** /login → post-auth redirect
- **Sub-surface:** global (login redirect destination)
- **Dimension:** B, H
- **Severity:** High
- **Affects:** all users
- **Status:** Open
...
**What:**
Post-login destination is hardcoded to `/dashboard` (`apps/dashboard/src/app/login/page.tsx:23`). The audit spec scopes `/console` as Surface 01's primary high-traffic route, but the operator never lands there from sign-in — they have to know to type `/console` in the URL bar. Combined with DC-40 (no in-page nav from /console out) and the fact that nothing in the dashboard nav links _into_ /console, the surface is effectively orphaned: unreachable by normal navigation, with no exit if you do reach it. Either /console is not actually the primary post-launch surface (in which case the audit's surface scoping is wrong and /dashboard should be Surface 01) or the post-login redirect should be /console.

**Evidence:**

- File: apps/dashboard/src/app/login/page.tsx:23 (`callbackUrl = searchParams.get("callbackUrl") || "/dashboard"`)
- File: apps/dashboard/src/app/login/page.tsx:29,48 (router.push / location redirect to that callbackUrl)
- Screenshot: artifacts/01-dashboard-core/screenshots/states/error-console-blank-body-repro.png (operator had to manually edit URL bar from `/dashboard` to `/console`)
- Repro:
  1. Sign in at `/login`. Observe automatic redirect to `/dashboard`, not `/console`.
  2. From `/dashboard`, scan the page and chrome for any link/tab/button that takes you to `/console`. There is none.
  3. The only way to reach `/console` is to manually edit the URL bar.

**Fix:**
Resolve at the spec level first: confirm whether `/console` or `/dashboard` is the intended post-launch primary surface. If `/console`, change `callbackUrl` default to `/console` and add a chrome-level link from `/dashboard` to `/console`. If `/dashboard`, the audit's surface scoping is wrong — /dashboard should be Surface 01 and /console rolls into Surface 02 or is descoped from launch entirely.
