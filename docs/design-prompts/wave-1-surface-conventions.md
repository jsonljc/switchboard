...
- **`--font-serif`** / `--serif` — Source Serif 4 stack (`apps/dashboard/src/app/globals.css:173-175`). **Agent-home prose only.** Consumed by `.greeting-prose` (`apps/dashboard/src/app/globals.css:707-715`), `.win-prose` (`apps/dashboard/src/app/globals.css:837-845`), `.hero-num` (`apps/dashboard/src/app/globals.css:879-886`), `.tile-name` (`apps/dashboard/src/app/globals.css:1106-1113`). Tools-tier surfaces (`/approvals`, `/reports`, `/activity`, `/mission`) lead with Inter, not serif.
...
Agent-home prose (cockpit + greeting) uses Source Serif 4 via `--serif` (`apps/dashboard/src/app/globals.css:173`; consumed at `.greeting-prose` line 708). The locked Tools-tier CSS modules (`locked/switchboard/project/approvals-v2/styles.css:36`, `reports-v2/styles.css:29`, `activity-v2/styles.css:42`) declare `--font-display: "Cormorant Garamond"`. **Source Serif 4 wins for agent-home prose**; Cormorant in the locked CSS is mockup-local display chrome, not a production directive. Tools-tier surfaces do not adopt Cormorant — they consume `--font-display` (Instrument Sans) per their surface specs.

### 2.2 Adjudication — Instrument Sans vs Cormorant Garamond (display)

`docs/design-prompts/2026-05-13-reports.md` explicitly directs Instrument Sans for the display title, hero number, and pull-quote value/cost. `docs/design-prompts/2026-05-13-approvals.md` and `docs/design-prompts/2026-05-13-mission.md` likewise direct Instrument Sans for headings. The locked CSS declares Cormorant Garamond as `--font-display` (`locked/switchboard/project/approvals-v2/styles.css:36`, `locked/switchboard/project/reports-v2/styles.css:29`, `locked/switchboard/project/activity-v2/styles.css:42`) — **spec wins**, mockup loses. Production code consumes `--font-display` (Instrument Sans) for all wave-2 display chrome.
...
- **Agent-context accents** — Alex amber `#B8782E` (`locked/switchboard/project/agent-home-v3/alex-config.jsx:8`) and Riley clay `#B86C50` (`locked/switchboard/project/agent-home-v3/riley-config.jsx:8`). Owned exclusively by their agent's context. Used for that agent's avatar frame (`AlexFrame` background via `AG.accent.soft` at `locked/switchboard/project/agent-home-v3/cockpit.jsx:101`), status-pill dot color when the agent is in a live state (Alex `WAITING` `#B8782E` at `alex-config.jsx:18-22`; Riley `REVIEWING`/`WAITING` `#B8782E` at `riley-config.jsx:20-26`), and the cockpit ROI-bar gradient (`AG.accent.soft → AG.accent.base` at `locked/switchboard/project/agent-home-v3/cockpit.jsx:487`). The shared `KIND_META` chip palette is fixed amber (`locked/switchboard/project/agent-home-v3/cockpit.jsx:26-44`) — those chips are not per-agent. **Alex amber NEVER appears on `/riley` and vice versa.** Cross-agent surfaces (`/mission`, `/approvals`, `/activity`, `/reports`) do not consume agent-context accents — they stay on `--sw-accent` / `--editorial-accent` per the rules above.
...
- **`--duration-default` 280ms** (`apps/dashboard/src/app/globals.css:127`) — the workhorse: row hover, filter-chip activation, button background swap, accordion expand, ring/border state change. `/approvals`, `/reports`, `/activity` declare this exact value (`locked/switchboard/project/approvals-v2/styles.css:40`, `locked/switchboard/project/reports-v2/styles.css:33`, `locked/switchboard/project/activity-v2/styles.css:46`) and consume it for transitions on rows, chips, badges, buttons. **Mission is the exception** — `locked/switchboard/project/mission/styles.css:45` declares `--duration-default: 240ms` (a hair faster, deliberate for the higher-density queue + agent strip). Mission's CSS module ships its own value; the wave-2 `/mission` implementation keeps the locked 240ms locally rather than overriding `globals.css`.
...
- **Don't use this doc to initiate a route-group migration or to force a surface to drop its `--mercury-*` aliases.** Because route-group and token-base migrations land as their own PRs, authorized by the surface's own spec — re-read authoring rule §4 and the §1.3 Layer-2 table. The conventions doc is descriptive of authorized destinations, not an authorization itself.
- **Don't put a shadcn `Card` shadow on any wave-2 surface.** Because paper-and-ink uses hairlines, not shadows. Every locked CSS module declares borders as 1px hair or hair-soft (`locked/switchboard/project/approvals-v2/styles.css:19-21`, `locked/switchboard/project/reports-v2/styles.css:18-20`, `locked/switchboard/project/activity-v2/styles.css:24-26`); none introduces `box-shadow` on block surfaces. The `.tp-panel` tweaks panel is the single internal exception (`apps/dashboard/src/app/globals.css:1191`) — that's a designer-tools overlay, not product chrome.
- **Don't use `--sw-accent` as a generic accent.** Because §4 enumerates exactly four authorized uses: primary mutating Approve CTA on `/approvals` (cite `docs/design-prompts/2026-05-13-approvals.md`); muted-amber numeric emphasis on `/reports` (cite `docs/design-prompts/2026-05-13-reports.md`); restricted active-filter chip + selected row on `/activity` (cite `docs/design-prompts/2026-05-13-activity.md`); the public marketing site. Any other use needs new surface-spec authorization.
- **Don't use Alex amber (`#B8782E`) outside Alex's agent context.** Because agent-context accents are owned by the agent and never bleed cross-surface (`locked/switchboard/project/agent-home-v3/alex-config.jsx:8`). Cross-agent surfaces use `--sw-accent` or `--editorial-accent` per §4, never `#B8782E`.
- **Don't use Riley clay (`#B86C50`) outside Riley's agent context.** Because agent-context accents are owned by their agent — Riley clay is bound to `/riley`'s identity (`locked/switchboard/project/agent-home-v3/riley-config.jsx:8`). The cockpit's ROI-bar gradient consumes Riley's `AG.accent.base` only when the rendering agent IS Riley (`locked/switchboard/project/agent-home-v3/cockpit.jsx:487`).
- **Don't use the editorial or operator accent for success states.** Because amber is "operator needs attention" — green is `--positive` `hsl(152 28% 32%)` (`apps/dashboard/src/app/globals.css:42`) or calibrated `#3F7A36` (`locked/switchboard/project/agent-home-v3/cockpit.jsx:21`). Cross-wires when amber doubles as success and as attention.
- **Don't use the editorial or operator accent for links unless explicitly noted in §4.** Because editorial links are underlined ink (`apps/dashboard/src/app/globals.css:856,866-876` `.win-undo`, `.see-all`). The only link-shaped accent uses authorized by §4 are the folio-link "pip" indicator (`apps/dashboard/src/app/globals.css:602`) and the funnel-byline marker on `/reports` (`locked/switchboard/project/reports-v2/styles.css:552`) — neither is a generic hyperlink.
- **Don't introduce a new `--duration-*` value inline.** Because §6 requires every new duration to land as a new token in `globals.css` in the same PR that consumes it. Inline `transition: ... 350ms ...` bypasses the token registry and is reviewer-rejected.
- **Don't invent specific values.** Because authoring rule §1.4.1 binds: every pixel height, motion duration, hairline weight, focus ring width, table row height, and color hex/HSL must come from a locked mockup or a surface spec — never from the implementer's head. The reviewer cross-checks every numeric value against its citation.
- **Don't write rules without a citation.** Because un-cited rules are reviewer-rejected (authoring rule §1.4.1). If a rule emerges mid-redesign without a citation, that's a signal it belongs in a focused conventions-doc PR back to `main`, not in the surface PR.
- **Don't add auto-refresh or a real-time tail to `/activity` by default.** Because `docs/design-prompts/2026-05-13-activity.md#api-capabilities` is explicit: "No polling — pagination breaks on autorefresh. Manual 'refresh' button only." A real-time toggle may be mentioned as a future affordance; it must not ship in the wave-2 rebuild.
- **Don't use a chart library on `/reports` or `/mission`.** Because both surface specs forbid it: `docs/design-prompts/2026-05-13-reports.md#anti-patterns` ("No chart library — funnel and attribution are custom SVG / grid."), `docs/design-prompts/2026-05-13-mission.md#anti-patterns` ("No charts. Mini funnel is just bars; no recharts/d3."). Custom SVG + CSS bars only.
- **Don't bury the `bindingHash` behind an accordion or click affordance on `/approvals`.** Because the binding hash is the integrity contract — `docs/design-prompts/2026-05-13-approvals.md#anti-patterns` ("Don't bury the binding hash behind an accordion.") and §9.1 above. Block 2 stays a persistent, fully visible card.
- **Don't fill row backgrounds with a risk-graded hue on `/approvals` or `/activity`.** Because risk is communicated by left-edge hairline weight (depth, not hue) per §3.4 and `locked/switchboard/project/approvals-v2/styles.css:319-328`. Filling the row with red/amber/etc. breaks the depth-not-hue rule and replaces the calibrated visual with traffic-light noise.
